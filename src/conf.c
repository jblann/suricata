...
ConfNode *ConfNodeLookupAnchor(const char *anchor, ConfNode *base) {
    ConfNode *node, *child;

    /* If the root node is null, start at the configuration root. */
    if (base == NULL) {
        base = ConfGetRootNode();
    }

    if (base->anchor != NULL && strcmp(base->anchor, anchor) == 0) {
        return base;
    }

    TAILQ_FOREACH(node, &base->head, next) {
...
void ConfRegisterTests(void)
{
    ...
    UtRegisterTest("ConfNodeDeepCopyTest", ConfNodeDeepCopyTest);
    UtRegisterTest("ConfNodeLookupAnchorTest", ConfNodeLookupAnchorTest);
}
