 ...
 */
static int DNP3DecodeApplicationObjects(const uint8_t *buf, uint32_t len,
    DNP3ObjectList *objects)
{
    ...
    while (len) {
        ...
        switch (object->range_code) {
            ...
            case 0x09:
            case 0x0b:
                SCLogNotice("Range code 0x%02x not yet implemented.",
                    object->range_code);
                goto done;
            default:
                SCLogInfo("Range code 0x%02x is reserved.", object->range_code);
                goto done;
        }
        ...
        if (!DNP3DecodeObject(header->group, header->variation, &buf, &len,
                object->prefix_code, object->start, object->count,
                object->items)) {
            goto done;
        }
    ...
    uint8_t *input, uint32_t input_len, void *local_data)
{
    ...
    if (buffer->len) {
        ...
        if (processed < 0) {
            SCLogInfo("Failed to process request link layer.");
            goto error;
        }
