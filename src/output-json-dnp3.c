...
static int JsonDNP3PrefixIsSize(uint8_t prefix_code)
{
    switch (prefix_code) {
        case 0x04:
        case 0x05:
...
#if 0 /* Not sure if it makes sense to log this. */
static json_t *JsonDNP3LogTransportHeader(DNP3TransportHeader th)
{
    ...
    json_object_set_new(thjs, "fin", json_boolean(DNP3_TH_FIN(th)));
    json_object_set_new(thjs, "fir", json_boolean(DNP3_TH_FIR(th)));
    json_object_set_new(thjs, "sequence", json_integer(DNP3_TH_SEQ(th)));
...
static json_t *JsonDNP3LogObjectItems(DNP3Object *object)
{
    ...
    TAILQ_FOREACH(item, object->items, next) {
        ...
        if (JsonDNP3PrefixIsSize(object->prefix_code)) {
        ...
        switch (DNP3_OBJECT_CODE(object->group, object->variation)) {
            ...
            case DNP3_OBJECT_CODE(2, 1): {
                ...
                json_object_set_new(js, "state", json_integer(point->state));
                break;
            }
            ...
            case DNP3_OBJECT_CODE(12, 1): {
                ...
                json_object_set_new(js, "qu", json_integer(point->qu));
                json_object_set_new(js, "cr", json_integer(point->cr));
                json_object_set_new(js, "tcc", json_integer(point->tcc));
 ...
 */
static json_t *JsonDNP3LogObjects(DNP3ObjectList *objects, uint8_t include_data)
{
    DNP3Object *object;
    ...
    TAILQ_FOREACH(object, objects, next) {
        ...
        if (object->items != NULL && !TAILQ_EMPTY(object->items)) {
            ...
            if (items != NULL) {
                json_object_set_new(objs, "items", items);
            }
        }
    ...
    const Packet *p, DNP3Transaction *dnp3tx)
{
    ...
    LogDNP3FileCtx *ctx = thread->dnp3log_ctx;
    ...
    Flow *f, void *state, void *tx, uint64_t tx_id)
{
    ...
    if (dnp3tx->has_request && dnp3tx->request_done) {
        JsonDNP3LogRequest(thread, p, dnp3tx);
    }
    if (dnp3tx->has_response && dnp3tx->response_done) {
