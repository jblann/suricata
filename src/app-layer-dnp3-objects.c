    ...
    uint32_t count, DNP3ObjectItemList *items)
{
    ...
    switch (DNP3_OBJECT_CODE(group, variation)) {
        ...
        case DNP3_OBJECT_CODE(50, 3):
            ...
            break;
        default:
            SCLogNotice("Unknown DNP3 object: %d:%d\n", group, variation);
#ifdef FAIL_ON_UNKNOWN_OBJECTS
            exit(1);
#endif
            break;
    }

    /* If we get here, a decoder was found but failed. */
    if (!rc) {
        SCLogNotice("Failed to decode object %d:%d", group, variation);
    }
