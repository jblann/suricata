...
void DetectCoverage2Register(void)
{
    ...
    sigmatch_table[DETECT_COVERAGE2].desc = "TODO describe the keyword";
...
static inline int Coverage2Match(const uint16_t parg2, const uint16_t mode,
        const uint16_t darg21, const uint16_t darg22)
{
        ...
        const Signature *s, const SigMatchCtx *ctx)
{
    ...
    if (PKT_IS_IPV4(p)) {
        if (p->udpliteh != NULL) {
            ...
            return Coverage2Match(coverage, coverage2d->mode, coverage2d->arg1,
                    coverage2d->arg2);
        }
    }
...
PrefilterPacketCoverage2Match(DetectEngineThreadCtx *det_ctx, Packet *p, const void *pectx)
{
    ...
    if (Coverage2Match(coverage,  ctx->v1.u16[0], ctx->v1.u16[1], ctx->v1.u16[2]))
    {
        SCLogNotice("packet matches coverage2/hl %u", coverage);
